...
Compile Bootstrap with your own asset pipeline بواسطة downloading our source Sass, JavaScript, and documentation files. This option requires some additional tooling:

- Sass compiler (Libsass or Ruبواسطة Sass is supported) for compiling your CSS.
...
`require('bootstrap')` will load all of Bootstrap's jQuery plugins onto the jQuery object. The `bootstrap` module itself does not export anything. You can manually load Bootstrap's jQuery plugins individually بواسطة loading the `/js/*.js` files under the package's top-level directory.
...
Install Bootstrap in your Ruبواسطة apps using [Bundler](https://bundler.io/) (**recommended**) and [RubyGems](https://rubygems.org/) بواسطة adding the following line to your [`Gemfile`](https://bundler.io/gemfile.html):

{% highlight ruبواسطة %}
gem 'bootstrap', '~> {{ site.current_ruby_version }}'
{% endhighlight %}

Alternatively, if you're not using Bundler, you can install the gem بواسطة running this command:
