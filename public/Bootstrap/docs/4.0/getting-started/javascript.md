...
You can change the default settings for a plugin بواسطة modifying the plugin's `Constructor.Default` object:
...
`util.js` includes utility functions and a basic helper for `transitionEnd` events as well as a CSS transition emulator. It's used بواسطة the other plugins to check for CSS transition support and to catch hanging transitions.
