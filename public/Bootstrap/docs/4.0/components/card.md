...
Card titles are used بواسطة adding `.card-title` to a `<h*>` tag. In the same way, links are added and placed next to each other بواسطة adding `.card-link` to an `<a>` tag.

Subtitles are used بواسطة adding a `.card-subtitle` to a `<h*>` tag. If the `.card-title` and the `.card-subtitle` items are placed in a `.card-body` item, the card title and subtitle are aligned nicely.
...
Card headers can be styled بواسطة adding `.card-header` to `<h*>` elements.
...
Cards can be organized into [Masonry](https://masonry.desandro.com/)-like columns with just CSS بواسطة wrapping them in `.card-columns`. Cards are built with CSS `column` properties instead of flexbox for easier alignment. Cards are ordered from top to bottom and left to right.
