...
Use `<a>`s or `<button>`s to create _actionable_ list group items with hover, disabled, and active states بواسطة adding `.list-group-item-action`. We separate these pseudo-classes to ensure list groups made of non-interactive elements (like `<li>`s or `<div>`s) don't provide a click or tap affordance.
...
You can activate a list group navigation without writing any JavaScript بواسطة simply specifying `data-toggle="list"` or on an element. Use these data attributes on `.list-group-item`.
...
{% highlight js %}
$('#myList a[href="#profile"]').tab('show') // Select tab بواسطة name
