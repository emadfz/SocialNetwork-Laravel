...
بواسطة default, any number of checkboxes and radios that are immediate sibling will be vertically stacked and appropriately spaced with `.form-check`.
...
Group checkboxes or radios on the same horizontal row بواسطة adding `.form-check-inline` to any `.form-check`.
...
Since Bootstrap applies `display: block` and `width: 100%` to almost all our form controls, forms will بواسطة default stack vertically. Additional classes can be used to vary this layout on a per-form basis.
...
Create horizontal forms with the grid بواسطة adding the `.row` class to form groups and using the `.col-*-*` classes to specify the width of your labels and controls.
