...
- Use `$body-bg` to set a `background-color` on the `<body>` (`#fff` بواسطة default).
...
Make a paragraph stand out بواسطة adding `.lead`.
...
Align terms and descriptions horizontally بواسطة using our grid system's predefined classes (or semantic mixins). For longer terms, you can optionally add a `.text-truncate` class to truncate the text with an ellipsis.
...
*Responsive typography* refers to scaling text and components بواسطة simply adjusting the root element's `font-size` within a series of media queries. Bootstrap doesn't do this for you, but it's fairly easy to add if you need it.
