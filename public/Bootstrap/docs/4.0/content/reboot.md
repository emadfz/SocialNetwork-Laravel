...
- The `<body>` also sets a global `font-family`, `line-height`, and `text-align`. This is inherited later بواسطة some form elements to prevent font inconsistencies.
...
- `<input>`s, `<select>`s, `<textarea>`s, and `<button>`s are mostly addressed بواسطة Normalize, but Reboot removes their `margin` and sets `line-height: inherit`, too.
...
The `<address>` element is updated to reset the browser default `font-style` from `italic` to `normal`. `line-height` is also now inherited, and `margin-bottom: 1rem` has been added. `<address>`s are for presenting contact information for the nearest ancestor (or an entire body of work). Preserve formatting بواسطة ending lines with `<br>`.
...
HTML5 adds [a new global attribute named `[hidden]`](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/hidden), which is styled as `display: none` بواسطة default. Borrowing an idea from [PureCSS](https://purecss.io/), we improve upon this default بواسطة making `[hidden] { display: none !important; }` to help prevent its `display` from getting accidentally overridden. While `[hidden]` isn't natively supported بواسطة IE10, the explicit declaration in our CSS gets around that problem.
