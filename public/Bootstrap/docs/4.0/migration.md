...
description: Bootstrap 4 is a major rewrite of the entire project. The most notable changes are summarized below, followed بواسطة more specific changes to relevant components.
group: migration
toc: true
---
...
- **Flexbox is enabled بواسطة default.** In general this means a move away from floats and more across our components.
- Switched from [Less](http://lesscss.org/) to [Sass](http://sass-lang.com/) for our source CSS files.
- Switched from `px` to `rem` as our primary CSS unit, though pixels are still used for media queries and grid behavior as device viewports are not affected بواسطة type size.
...
- **Updated grid sizes, mixins, and variables.**
  ...
  - Changed grid system media query breakpoints and container widths to account for new grid tier and ensure columns are evenly divisible بواسطة `12` at their max width.
...
This list highlights key changes بواسطة component between v3.x.x and v4.0.0.
...
- Navbars no longer include `margin-bottom` or `border-radius` بواسطة default. Use utilities as necessary.
...
Rather than using explicit `.visible-*` classes, you make an element visible بواسطة simply not hiding it at that screen size. You can combine one `.d-*-none` class with one `.d-*-block` class to show an element only on a given interval of screen sizes (e.g. `.d-none.d-md-block.d-xl-none` shows the element only on medium and large devices).
