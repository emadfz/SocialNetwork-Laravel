...
// media types accepted بواسطة plugins
mejs.plugins = {
	silverlight: [
...
mejs.Utility = {
	...
	/*
	 * Prefix the given number بواسطة zero if it is lower than 10.
	 */
...
/*
Mimics the <video/audio> element بواسطة calling Flash's External Interface or Silverlights [ScriptableMember]
*/
...
mejs.PluginMediaElement.prototype = {
	...
	// HTML5 get/set properties, but only set (updated بواسطة event handlers)
	muted: false,
	volume: 1,
	currentTime: 0,
		...
		if (supportsMediaTag && (options.mode === 'auto' || options.mode === 'auto_plugin' || options.mode === 'native')  && !(mejs.MediaFeatures.isBustedNativeHTTPS && options.httpsBasicAuthSite === true)) {
			...
			for (i=0; i<mediaFiles.length; i++) {
				...
				if (mediaFiles[i].type == "video/m3u8" || htmlMediaElement.canPlayType(mediaFiles[i].type).replace(/no/, '') !== ''
					...
					// special case for m4a supported بواسطة detecting mp4 support
...
(function ($) {
	...
	mejs.MepDefaults = {
		...
		audioHeight: -1,
		// initial volume when the player starts (overrided بواسطة user cookie)
		startVolume: 0.8,
	...
	mejs.MediaElementPlayer.prototype = {
		...
		meReady: function(media, domNode) {
			...
			if (!(mf.isAndroid && t.options.AndroidUseNativeControls) && !(mf.isiPad && t.options.iPadUseNativeControls) && !(mf.isiPhone && t.options.iPhoneUseNativeControls)) {
				...
				//	user has to start playback directly بواسطة tapping on the iFrame.
		...
		remove: function() {
			...
			if (!t.isDynamic) {
				...
				//		 also detach ONLY the events attached بواسطة this plugin!
...
(function($) {
	...
	$.extend(mejs.MepDefaults, {
		...
		// بواسطة default, no WAI-ARIA live region - don't make a
