...
@section('content')
    <style>
        .page_intro.row .inner:before {
            background: url('{{ asset('public/uploads/full/' . $burn->media->guid) }}') no-repeat scroll left center;
    ...
    <section class="row quick_blocks_row quick_blocks_row2">
        <div class="container">
            <div class="row">
                @if(empty($burn->post_q1 ||$burn->post_q2||$burn->post_q3||$burn->post_q4||$burn->post_q5))
                <div style="display: none"></div>


                @else
                    <h3> {{trans('main.fa')}}</h3>
                    @if(empty($burn->post_q1))
